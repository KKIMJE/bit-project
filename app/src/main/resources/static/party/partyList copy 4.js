...
function computeDistance() {
  ...
  const addressSearch = address => { // 주소를 넣으면
      return new Promise((resolve, reject) => { // address가 들어가서 위도/경도 값을 찾았으면 resolve에 담아 리턴되고 아니라면 status가 반환된다.
    ...
    (async () => { // 익명함수
      try {
          ...
          // 만약 쿼리스트링을 ?로 잘랐을 때 길이가 1이면 현위치 함수를 실행하고, 
          // 아니라면 설정된 주소의 위도 경도 값을 geoResult에 담는다.

          let geoResult;
          var arr = location.href.split("?");

          if (arr.length == 1) {
            geoResult = await geoLocation() // 현위치
