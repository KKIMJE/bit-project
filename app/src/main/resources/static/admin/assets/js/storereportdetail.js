...
let repDetailDiv = document.querySelector(".repdetail-container")
...
fetch(`/report/get?no=${no}`)
  ...
  .then(result => {
    ...
    <td style="text-align:left;">주점신고번호: ${report.repoNo}</td>
    </tr>
    <tr>
    ...
    <button type="button" onclick="location.href='boardreport.html'">나가기</button>
    </div>
    </div>
`
    ...
    if (window.confirm("정말 제재하시겠습니까?")) {
      fetch(`/report/update?no=${e.target.value}`)
        ...
        .then(result => {
          ...
          if (result.status == "success") {
            alert("제재에 성공하였습니다.")
            location.href="storereport.html"
          } else {
            alert(result.data);
          }
        })
