...
CREATE TABLE member (
  ...
  join_date         DATE               NOT NULL COMMENT '가입일시', -- 가입일시
  social_accept     <데이터 타입 없음> NOT NULL COMMENT '소셜회원여부', -- 소셜회원여부
  gender            <데이터 타입 없음> NOT NULL COMMENT '성별', -- 성별
  birth             INTEGER            NOT NULL COMMENT '생년월일', -- 생년월일
  self_introduction <데이터 타입 없음> NULL     COMMENT '자기소개', -- 자기소개
  ...
  block_date        DATE               NULL     COMMENT '제재일', -- 제재일
  block_accept      <데이터 타입 없음> NOT NULL COMMENT '제재여부', -- 제재여부
...
CREATE TABLE store (
  ...
  introduction             <데이터 타입 없음> NULL     COMMENT '가게소개', -- 가게소개
  evaluation_score         FLOAT              NULL     COMMENT '주점별점', -- 주점별점
  reservation_accept       <데이터 타입 없음> NOT NULL COMMENT '예약가능여부', -- 예약가능여부
  ...
  oper                     <데이터 타입 없음> NOT NULL COMMENT '영업여부', -- 영업여부
  status                   <데이터 타입 없음> NOT NULL COMMENT '상태' -- 상태
)
COMMENT '주점';
...
CREATE TABLE alcohol_management (
  ...
  mno      INTEGER     NOT NULL COMMENT '회원번호', -- 회원번호
  date     DATE        NULL     COMMENT '음주일시', -- 음주일시
...
CREATE TABLE reservation (
  ...
  store_no       INTEGER      NOT NULL COMMENT '주점번호', -- 주점번호
  date           DATE         NOT NULL COMMENT '예약일시', -- 예약일시
  ...
  pay_date       DATE         NOT NULL COMMENT '결제일', -- 결제일
...
CREATE TABLE inquiry (
  ...
  mno         INTEGER            NOT NULL COMMENT '회원번호', -- 회원번호
  date        DATE               NOT NULL COMMENT '문의일자', -- 문의일자
  type        INTEGER            NOT NULL COMMENT '문의유형', -- 문의유형
  title       VARCHAR(50)        NOT NULL COMMENT '제목', -- 제목
  contents    <데이터 타입 없음> NOT NULL COMMENT '내용', -- 내용
  attach_file VARCHAR(500)       NULL     COMMENT '첨부파일', -- 첨부파일
  answer      <데이터 타입 없음> NULL     COMMENT '답변', -- 답변
  answer_date DATE               NULL     COMMENT '답변일시' -- 답변일시
)
COMMENT '문의';
...
CREATE TABLE notice (
  ...
  reg_date    DATE               NOT NULL COMMENT '작성일시', -- 작성일시
  update_date DATE               NOT NULL COMMENT '수정일시', -- 수정일시
  contents    <데이터 타입 없음> NOT NULL COMMENT '내용', -- 내용
...
CREATE TABLE store_menu (
  ...
  main_accept   <데이터 타입 없음> NOT NULL COMMENT '대표메뉴여부' -- 대표메뉴여부
)
COMMENT '메뉴';
...
CREATE TABLE alcohol_detail (
  ...
  characteristic    <데이터 타입 없음> NULL     COMMENT '특징', -- 특징
...
CREATE TABLE review (
  ...
  store_no         INTEGER            NOT NULL COMMENT '주점번호', -- 주점번호
  contents         <데이터 타입 없음> NOT NULL COMMENT '내용', -- 내용
  reg_date         DATE               NOT NULL COMMENT '게시일', -- 게시일
  score            FLOAT              NOT NULL COMMENT '별점', -- 별점
  comment_contents <데이터 타입 없음> NULL     COMMENT '답변', -- 답변
  comment_reg_date DATE               NULL     COMMENT '답변일' -- 답변일
)
COMMENT '리뷰';
...
CREATE TABLE board (
  ...
  reg_date     DATE         NOT NULL COMMENT '게시일', -- 게시일
  update_date  DATE         NOT NULL COMMENT '수정일', -- 수정일
  view_count   INTEGER      NOT NULL COMMENT '조회수' -- 조회수
)
COMMENT '게시글';
...
CREATE TABLE report (
  ...
  date      DATE               NOT NULL COMMENT '신고일자', -- 신고일자
  contents  <데이터 타입 없음> NULL     COMMENT '신고내용', -- 신고내용
  status    <데이터 타입 없음> NOT NULL COMMENT '처리상태' -- 처리상태
)
COMMENT '신고하기';
...
CREATE TABLE party_board (
  ...
  mno            INTEGER            NOT NULL COMMENT '회원번호', -- 회원번호
  board_reg_date DATE               NOT NULL COMMENT '게시판등록일', -- 게시판등록일
  message        <데이터 타입 없음> NULL     COMMENT '메시지', -- 메시지
  message_time   DATE               NULL     COMMENT '메시지시간', -- 메시지시간
  message_status <데이터 타입 없음> NOT NULL COMMENT '상태' -- 상태
)
COMMENT '모임게시판';
...
CREATE TABLE party (
  ...
  meeting_date  DATE         NOT NULL COMMENT '모임일시', -- 모임일시
  ...
  view_count    DATE         NOT NULL COMMENT '조회수', -- 조회수
...
CREATE TABLE board_comment (
  board_commnet_no INTEGER            NOT NULL COMMENT '게시글댓글번호', -- 게시글댓글번호
  mno              INTEGER            NOT NULL COMMENT '회원번호', -- 회원번호
  comment_contents <데이터 타입 없음> NOT NULL COMMENT '내용', -- 내용
  comment_date     DATE               NOT NULL COMMENT '작성일', -- 작성일
  update_date      DATE               NOT NULL COMMENT '수정일' -- 수정일
)
COMMENT '게시글댓글';
