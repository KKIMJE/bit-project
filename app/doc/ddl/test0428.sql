...
CREATE TABLE `member` (
  ...
  `birth` int(11) DEFAULT NULL COMMENT '생년월일',
  ...
  `block_date` timestamp NULL DEFAULT NULL COMMENT '제재일',
  `block_accept` tinyint(1) NOT NULL DEFAULT 0 COMMENT '제재여부',
  `member_status` tinyint(1) NOT NULL DEFAULT 0 COMMENT '탈퇴여부(0-회원,1-탈퇴)',
  PRIMARY KEY (`mno`),
  UNIQUE KEY `UIX_member` (`email`),
  UNIQUE KEY `UIX_member2` (`tel`),
  UNIQUE KEY `UIX_member3` (`nickname`)
) ENGINE=InnoDB AUTO_INCREMENT=65 DEFAULT CHARSET=utf8mb3 COMMENT='회원';
...
CREATE TABLE `tag` (
  ...
  PRIMARY KEY (`tag_no`)
) ENGINE=InnoDB AUTO_INCREMENT=35 DEFAULT CHARSET=utf8mb3 COMMENT='태그';
...
) ENGINE=InnoDB AUTO_INCREMENT=291 DEFAULT CHARSET=utf8mb3 COMMENT='술';
...
CREATE TABLE `party` (
  ...
  `writer` int(11) NOT NULL COMMENT '주최자번호',
  `title` varchar(50) NOT NULL COMMENT '제목',
  `contents` longtext NOT NULL COMMENT '내용',
  `party_fee` varchar(20) NOT NULL COMMENT '회비',
  `meeting_date` varchar(30) NOT NULL COMMENT '모임일시',
  `max_member` varchar(20) NOT NULL DEFAULT '1' COMMENT '최대인원',
  `alcohol_type` varchar(20) NOT NULL COMMENT '주종',
  `alcohol_limit` varchar(20) NOT NULL COMMENT '주량',
  `reg_date` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '등록일',
  `update_date` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT '수정일',
  `address` varchar(100) NOT NULL COMMENT '모임위치',
  PRIMARY KEY (`party_no`),
  KEY `FK_member_TO_party` (`writer`),
  CONSTRAINT `FK_member_TO_party` FOREIGN KEY (`writer`) REFERENCES `member` (`mno`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=46 DEFAULT CHARSET=utf8mb3 COMMENT='모임';
...
CREATE TABLE `party_comment` (
  `party_comment_no` int(11) NOT NULL AUTO_INCREMENT COMMENT '모임댓글번호',
  `commentWriter` int(11) NOT NULL COMMENT '회원번호',
  ...
  KEY `FK_member_TO_party_comment` (`commentWriter`),
  KEY `FK_party_TO_party_comment` (`party_no`),
  CONSTRAINT `FK_member_TO_party_comment` FOREIGN KEY (`commentWriter`) REFERENCES `member` (`mno`) ON DELETE CASCADE,
  CONSTRAINT `FK_party_TO_party_comment` FOREIGN KEY (`party_no`) REFERENCES `party` (`party_no`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=56 DEFAULT CHARSET=utf8mb3 COMMENT='모임댓글';
...
CREATE TABLE `party_board` (
  ...
  `party_no` int(11) DEFAULT NULL COMMENT '모임번호',
  `sender` int(11) NOT NULL COMMENT '회원번호',
  `board_reg_date` timestamp NULL DEFAULT current_timestamp() COMMENT '게시판등록일',
  ...
  KEY `FK_party_participant_TO_party_board` (`party_no`,`sender`),
  CONSTRAINT `FK_party_participant_TO_party_board` FOREIGN KEY (`party_no`, `sender`) REFERENCES `party_participant` (`party_no`, `mno`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=108 DEFAULT CHARSET=utf8mb3 COMMENT='모임게시판';
...
CREATE TABLE `party_select` (
  ...
  CONSTRAINT `FK_member_TO_party_select` FOREIGN KEY (`mno`) REFERENCES `member` (`mno`) ON DELETE CASCADE,
  CONSTRAINT `FK_party_TO_party_select` FOREIGN KEY (`party_no`) REFERENCES `party` (`party_no`) ON DELETE CASCADE
